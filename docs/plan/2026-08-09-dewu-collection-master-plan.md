...
除 2.4 节明确标注的当前验证快照外，这些数量只属于本次运行报告、任务状态或日志，不回写到本总规划，也不作为下一次运行的固定基准。不同库存文件的数据量可以不同，但去重、采集、落盘、验收规则保持一致。
...
2026-08-09 对当前真实库存表 `data/cost data/0123-0223促销库存(4).xlsx` 的只读检查结果为：`Sheet1!A1:P12319`，共 12,318 条库存尺码明细，A 列无空货号，去重后有 2,978 个唯一货号。该数量只用于本轮实现和验收，不替代 2.2 节的动态基线规则。

`data/0123-0223促销库存(4)_随机样本50.xlsx` 只有原库存表的 A–P 列和 50 个唯一货号，不包含得物商品名、得物价格、销量或接口响应。该文件只能作为输入解析、50 条批量运行和性能测试样本，不得被表述为已验证的得物采集结果。
...
当前 `Medium_Phone` 是 Android 15 `google_apis_playstore`、arm64-v8a、`user/release-keys` 镜像，`ro.debuggable=0`。为避免破坏现有得物安装和登录环境，保留该 AVD，不对它执行 Magisk Root；另建可脚本化重建的 Android 15 `google_apis` arm64 Root AVD。新环境由用户手动登录一次，不复制当前设备私有数据。
...
5. **M4：可移植打包**。增加统一 CLI、Node/Frida/SDK lock、跨平台路径和安装说明；在第二台干净兼容电脑上重建环境并运行同一 50 条任务。
6. **M5：全量运行**。通过 50 条准确性和性能验收后，对当前 2,978 个唯一货号执行可恢复全量采集。

### 9.7 测试与产品化验收

1. 脱敏 fixture 单元测试覆盖鞋子、服装、配件、无结果、无报价、特殊尺码、分页、字段缺失和 Schema 漂移。
2. 至少 12 个跨类别货号在同一登录态和短时间窗口内进行接口响应与得物 UI 对照；第一条商品、商品名、全部显示尺码、价格和销量原文必须一致。
3. 使用现有 50 条库存样本完成批量运行，正式循环中不得发生逐货号 UI 操作；随机抽查至少 10 条与 UI 一致。
4. 会话准备完成后，未触发服务端限流时，50 条样本应在 10 分钟内完成，并且相对现有 UI 流程至少快 5 倍。
...
7. 在当前电脑和另一台干净兼容电脑上从环境引导开始完成同一 50 条任务；配置中不得出现当前用户名、固定 serial、固定 AVD 名称或 SDK 绝对路径。
...
| `MOD-04 Jobs` | Excel 输入、动态基线、任务状态、自适应并发、重试和断点 | `src/jobs/` | `MOD-00` | 12,318 行/2,978 货号基线、恢复和并发状态机测试通过 |
...
| `MOD-07 Integration` | 统一 `dewu` CLI、模块装配、数据流和端到端运行 | `src/cli/`、`src/integration/` | `MOD-03`、`MOD-04`、`MOD-05`、`MOD-06` | 12 条同刻对照和 50 条响应级运行通过 |
...
| `W4 Integration` | `MOD-07` | `MOD-03`、`MOD-04`、`MOD-05`、`MOD-06` 全部合并 | 统一 CLI、12 条同刻准确性和 50 条端到端运行通过 |
| `W5 QA & Portability` | `MOD-08` | `MOD-07` 合并 | 故障注入、断点、性能、秘密扫描、第二台电脑重建全部通过 |
| `W6 Full Run` | 只执行已合并版本，不新增生产模块 | `MOD-08` 发布门通过 | 当前 2,978 个唯一货号完成或以可解释阻塞状态收口 |
...
- `MOD-07` 合并前必须确认所有上游模块已合并且 contract 版本一致；`MOD-08` 发布前必须确认 12 条同刻对照、50 条性能、故障恢复、秘密扫描和第二台电脑重建证据齐全。
